...
if sayfa == "Ana Panel":
    ...
    if st.button("💰 GÜNÜ KAPAT"):
        kayit = {
            ...
            "Kripto (TL)": f"₺{res_k['tl']:,.0f}", "Nakit (TL)": f"₺{res_n['tl']:,.0f}",
            "Borsa (TL)": f"₺{res_h['tl']:,.0f}", "Toplam (TL)": f"₺{g_tl:,.0f}",
...
# --- DİĞER SAYFALAR (GEÇMİŞ, BÜTÇE VS.) ---
    ...
    if st.button("💾 ARŞİVLE"):
        b_k = {"tarih": datetime.now().strftime("%Y-%m-%d %H:%M"), "GELİR (TL)": f"₺{t_gel:,.0f}", "GİDER (TL)": f"₺{t_gid:,.0f}", "NET (TL)": f"₺{net:,.0f}", "NET ($)": f"${net/usd_val:,.0f}", "Değişim %": "0.00%"}
